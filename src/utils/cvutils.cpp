  ...
  for (int y = 0; y < mask.rows; ++y)
    for (int x = 0; x < mask.cols; ++x) {
      if (mask.at<uchar>(y, x) > 0) {
        ...
        vec[0] = alpha * color[0] + (1 - alpha) * vec[0];
        vec[1] = alpha * color[1] + (1 - alpha) * vec[1];
        vec[2] = alpha * color[2] + (1 - alpha) * vec[2];
      }
    }
}
...
void write_depth(const std::string &depth_file, const cv::Mat &depth_meter) {
  ...
  for (size_t y = 0; y < depth_mat.rows; y++)
    for (size_t x = 0; x < depth_mat.cols; x++) {
      unsigned short depth_short = std::round(depth_meter.at<float>(y, x) * 10000.);
...
cv::Mat read_depth(const std::string &depth_file) {
  ...
  for (size_t y = 0; y < depth_mat.rows; ++y)
    for (size_t x = 0; x < depth_mat.cols; ++x) {
      ...
      depth_short = (depth_short << 13 | depth_short >> 3);
      depth_meter.at<float>(y, x) = (double) depth_short / 10000.;
    }
  return depth_meter;
}
