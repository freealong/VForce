...
bool RandomICPMatcher::LoadConfig(const std::string &cfg_file) {
  ...
  DLOG(INFO) << "model_path: " << model_path;
  cv::FileNode model_size = fs["model_size"];
  model_size["x"] >> model_size_[0];
  model_size["y"] >> model_size_[1];
  model_size["z"] >> model_size_[2];
  DLOG(INFO) << "model_size: x: " << model_size_[0]
             << ", y: " << model_size_[1]
             << ", z: " << model_size_[2];
  ...
  fs["ter_num"] >> iter_num_;
  DLOG(INFO) << "iter_num_: " << iter_num_;
  // read model and sampling
  if (pcl::io::loadPCDFile(model_path, *model_) == -1) {
    LOG(ERROR) << "Load object model failed: " << model_path;
    return false;
  }
  Utils::sampling_cloud(model_, sampled_model_, uniform_radius_);
  init_ = true;
  return true;
}

double RandomICPMatcher::EstimatePose(const PointTCloudPtr &target, Eigen::Matrix4f &tf, PointTCloudPtr &final) {
  if (!init_) {
...
//  icp.setEuclideanFitnessEpsilon(0.00001);
  ...
  icp.setInputSource(sampled_model_);
  ...
  transformPointCloud(*sampled_model_, *final, tf);
  ...
  tf(1, 3) += center.y;
  tf(2, 3) += center.z;
  pcl::transformPointCloud(*sampled_model_, *final, tf);
