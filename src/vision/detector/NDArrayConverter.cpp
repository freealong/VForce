...
cv::Mat NDArrayConverter::toMat(const PyObject *o) {
  ...
  PyArrayObject *array_o = (PyArrayObject *) (o);
  int typenum = PyArray_TYPE(array_o);
  ...
  int ndims = PyArray_NDIM(array_o);
  ...
  const npy_intp *_sizes = PyArray_DIMS(array_o);
  const npy_intp *_strides = PyArray_STRIDES(array_o);
  ...
  m = Mat(ndims, size, type, PyArray_DATA(array_o), step);
